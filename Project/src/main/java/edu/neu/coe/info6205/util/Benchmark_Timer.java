...
public class Benchmark_Timer<T> implements Benchmark<T> {
    ...
    public static void main(String[] args) {
        int[] size = {5, 5, 5, 10, 50, 100, 1000, 2000, 5000};

        for(int i = 0; i < size.length; i++){
            System.out.println("*************************");
            System.out.println("Current array size: " + size[i]);
            Integer[] orderedArray = getOrderArray(size[i]);
            sort(orderedArray, "Ordered Array ");

            Integer[] partialOrderedArray = getShuffleArray(size[i], 0, size[i] / 2);
            sort(partialOrderedArray, "Partial Ordered Array ");

            Integer[] randomArray = getShuffleArray(size[i], 0, size[i] - 1);
            sort(randomArray, "Random Array ");

            Integer[] reverseOrderArray = getReversedArray(size[i]);
            sort(reverseOrderArray, "Reverse Ordered Array ");
        }
    }

    private static void sort(Integer[] nums, String str){
        //System.out.println(str + " " + Arrays.toString(nums));
        SortWithHelper<Integer> sorter = new InsertionSort<Integer>();
        sorter.preProcess(nums);
        Integer[] nums2 = sorter.sort(nums);
        final Timer timer = new Timer();

        final int zzz = 20;
        final double mean = timer.repeat(80, () -> zzz, t -> {
            Integer[] array3 = sorter.sort(nums);
            return null;
        });
    ...
    private static Integer[] getShuffleArray(int n, int start, int end){
        Integer[] result = new Integer[n];
